...
def _segm_resnet(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    ...
    if num_channels == 3:
        ...
        input_layer = nn.Conv2d(num_channels, 3, kernel_size=1)
    ...
    model = DeepLabV3(num_channels, backbone, classifier)
    return model
...
def _segm_mobilenet(name, backbone_name, num_classes, output_stride, pretrained_backbone, num_channels):
    if output_stride==8:
        aspp_dilate = [12, 24, 36]
    else:
        aspp_dilate = [6, 12, 18]
    ...
    model = DeepLabV3(backbone, classifier, input_layer)
    return model

def _load_model(arch_type, backbone, num_classes, output_stride, pretrained_backbone, num_channels=3):

    if backbone=='mobilenetv2':
        model = _segm_mobilenet(arch_type, backbone, num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone, num_channels=num_channels)
    elif backbone.startswith('resnet'):
        model = _segm_resnet(arch_type, backbone, num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone, num_channels=num_channels)
...
def deeplabv3plus_resnet101(num_classes=21, output_stride=8, pretrained_backbone=True, num_channels=3):
    """Constructs a DeepLabV3+ model with a ResNet-101 backbone.
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return _load_model('deeplabv3plus', 'resnet101', num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone, num_channels=num_channels)


def deeplabv3plus_mobilenet(num_classes=21, output_stride=8, pretrained_backbone=True, num_channels=3):
    """Constructs a DeepLabV3+ model with a MobileNetv2 backbone.
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return _load_model('deeplabv3plus', 'mobilenetv2', num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone, num_channels=num_channels)
